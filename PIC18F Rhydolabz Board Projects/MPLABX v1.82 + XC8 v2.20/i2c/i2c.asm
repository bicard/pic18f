...
SSPIF_CHK   BTFSS PIR1,SSPIF	;wait for data to be read(check SSP interrupt flag)
			BRA SSPIF_CHK		;skip if set
...
I2CSTART
	...
	START_CHK   BTFSS PIR1,SSPIF	;wait for START enable(check SSP interrupt flag)
				BRA START_CHK		;skip if set
				BCF PIR1,SSPIF		;clear flag bit
				RETURN				;return from subroutine

I2CSTOP
				BSF SSPCON2,PEN		;STOP Condition Enable Bit
	STOP_CHK    BTFSS PIR1,SSPIF	;wait for STOP condition(check SSP interrupt flag)
				BRA STOP_CHK		;skip if set
				BCF PIR1,SSPIF		;clear flag bit
				RETURN				;return from subroutine

VERIFY:
				MOVWF SSPBUF		;loading data to SSPBUF for i2c write
    VRFY_CHK    BTFSS PIR1,SSPIF	;wait for data write(check SSP interrupt flag)
				BRA VRFY_CHK    	;skip if set
				BCF PIR1,SSPIF		;clear flag bit
